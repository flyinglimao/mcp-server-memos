...
export function registerWebhookTools(server: McpServer): void {
        ...
        async ({ instance, user }) => {
            ...
            try {
                ...
                const list = response.webhooks
                    .map(w => `- **${w.displayName || 'No display name'}**\n  Name: \`${w.name}\`\n  URL: ${w.url}`)
                    .join('\n');
    ...
    server.tool(
        ...
        async ({ instance, webhookName, url, displayName }) => {
            ...
            if (url !== undefined) { request.url = url; updateFields.push('url'); }
            if (displayName !== undefined) { request.displayName = displayName; updateFields.push('displayName'); }
