  ...
  server.tool(
    ...
    'List memos from one or all connected Memos instances.',
    {
      instance: z
        .string()
        .optional()
        .describe('Instance name to query. If not specified, queries all instances.'),
      filter: z
        .string()
        .optional()
        .describe('CEL filter expression (e.g., "tag == \\"important\\"")'),
      pageSize: z
        .number()
        .optional()
        .default(20)
        .describe('Number of memos to return (default: 20, max: 1000)'),
